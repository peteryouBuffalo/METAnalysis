...
void ZxSelector::SlaveBegin(TTree * /*tree*/)
{
   ...
   // Create plots that hold the MET and # of jets for Z+l events
   ...
   // Create plots that hold the MET and # of jets for Z+b events
   ...
   // Create plots that hold the MET and # of jets for Z+c events
   ...
   // Create plots that hold the reconstructed di-lepton mass, 
   // separately for muons & electrons (and each jet type)
   ...
   h_Zee_cJet = new TH1F("Z_ee_mass", "Di-electron mass", 25, 69.5, 119.5);
   h_Zee_cJet->SetXTitle("m_{ee} [GeV]"); h_Zee_cJet->SetYTitle("Events/2 GeV");
   cHists_noT.push_back(h_Zee_cJet);
         
   h_Zmm_cJet = new TH1F("Z_mm_mass", "Di-muon mass", 25, 69.5, 119.5);
   h_Zmm_cJet->SetXTitle("m_{#mu#mu} [GeV]"); h_Zmm_cJet->SetYTitle("Events/2 GeV");
   cHists_noT.push_back(h_Zmm_cJet);
                  
   
   // Create histograms for the secondary vertex masses
   h_msv_lJet = new TH1F("mSV", "Secondary Vertex Mass", 25, -0.5, 4999.5);
   h_msv_lJet->SetXTitle("m_{SV} [MeV]"); h_msv_lJet->SetYTitle("Events/200 MeV");
   lHists_noT.push_back(h_msv_lJet);
   
   h_msv_bJet = new TH1F("mSV", "Secondary Vertex Mass", 25, -0.5, 4999.5);
   h_msv_bJet->SetXTitle("m_{SV} [MeV]"); h_msv_bJet->SetYTitle("Events/200 MeV");
   bHists_noT.push_back(h_msv_bJet);  
   
   h_msv_cJet = new TH1F("mSV", "Secondary Vertex Mass", 25, -0.5, 4999.5);
   h_msv_cJet->SetXTitle("m_{SV} [MeV]"); h_msv_cJet->SetYTitle("Events/200 MeV");
   cHists_noT.push_back(h_msv_cJet);
   
   // Create histograms to hold the MET reconstructed from the final
   // state particles
   h_METRec_lJet = new TH1F("MET_Rec", "MET Analysis (Reconstructed)", 
      150,	-0.5, 299.5); h_METRec_lJet->SetXTitle("MET [GeV]");
   h_METRec_lJet->SetYTitle("Events/2 GeV");
   lHists.push_back(h_METRec_lJet);
    
   h_METRec_bJet = new TH1F("MET_Rec", "MET Analysis (Reconstructed)",
      150,  -0.5, 299.5); h_METRec_bJet->SetXTitle("MET [GeV]");
   h_METRec_bJet->SetYTitle("Events/2 GeV");
   bHists.push_back(h_METRec_bJet);
                         
   h_METRec_cJet = new TH1F("MET_Rec", "MET Analysis (Reconstructed)", 
   150, -0.5, 299.5); h_METRec_cJet->SetXTitle("MET [GeV]");
   h_METRec_cJet->SetYTitle("Events/2 GeV");
   cHists.push_back(h_METRec_cJet);
...
Bool_t ZxSelector::Process(Long64_t entry)
{
   ...
   // The return value is currently not used.
  	...
  	if (Jets.size() == 0) return false;
   	...
   	if (elecTrig)
   	{
		if (Electrons.size() >= 2 && Electrons[0].m_lvec.Pt() 
			>= CUTS.Get<float>("lep_pt0"))
		{
   	...
   	if (muonTrig)
   	{
		if (Muons.size() >= 2 && Muons[0].m_lvec.Pt() >= CUTS.Get<float>("lep_pt0"))
		{
			...
			if (mZ >= CUTS.Get<float>("ZMassL") && mZ <= CUTS.Get<float>("ZMassH"))
			{
				...
				{ h_Zmm_bJet->Fill(mZ); }
				else if (cJets.size() > 0 && bJets.size() == 0 && lJets.size() == 0)
				{ h_Zmm_cJet->Fill(mZ); }
	 			else if (lJets.size() > 0 && bJets.size() == 0 && cJets.size() == 0)
				{ h_Zmm_lJet->Fill(mZ); }
			}
		}	
   	}
   	...
   	for (Int_t i = 0; i < Electrons.size(); ++i)
   		sum += Electrons.at(i).m_lvec;
   	for (Int_t j = 0; j < Muons.size(); ++j)
   		sum += Muons.at(j).m_lvec;
  	...
  	if (bJets.size() > 0) 
	{
		...
		// able to identify the SV, add its mass to the histogram
		for (Int_t i = 0; i < bJets.size(); ++i)
		{
			float msv = bJets.at(i).m_mSV;
  	...
  	if (cJets.size() > 0)
  	{
  		...
  		// For each jet, check the secondary vertex. If we were
  		// able to identify the SV, add its mass to the histogram
  		for (Int_t i = 0; i < cJets.size(); ++i)
  		{
  			float msv = cJets.at(i).m_mSV;
   	...
   	if (lJets.size() > 0)
   	{
		...
		// For each jet, check the secondary vertex. If we were
		// able to identify the SV, add its mass to the historam
		for (Int_t i = 0; i < lJets.size(); ++i)
		{
			float msv = lJets.at(i).m_mSV;
			if (msv >= 0) h_msv_lJet->Fill(msv*1000);
		}
   	}
